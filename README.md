...
## 6. Quick Start: SUMO Full Simulation with TripInfo Evaluation
...
./build/Simulation_Prediction \
  ...
  --sumo-net /data5/zhiyuan/roadnet_simulation/test.net.xml \
  --sumo-route /data5/zhiyuan/roadnet_simulation/high.rou.xml \
  --sumo-tripinfo /data5/zhiyuan/roadnet_simulation/tripinfo.xml \
  --travel-time-mode speed-net \
  --read-num 1000 \
  --eval-output /data5/zhiyuan/roadnet_simulation/sumo_eval.csv
```

SUMO `tripinfo.xml` is generated by SUMO or SUMO GUI with `--tripinfo-output`. The evaluator treats each `<tripinfo>` record's `duration` attribute as SUMO ground-truth travel time and uses `arrival` as a secondary arrival-time check. Records are aligned by SUMO vehicle id, not XML line order, so `veh_0`, `veh_1`, and other ids can appear in any order in `tripinfo.xml`.

If `--no-eval` is provided, SUMO truth evaluation is skipped even when `--sumo-tripinfo` is present. Use `--eval-output` to write a per-vehicle CSV containing predicted depart/arrival/duration, SUMO truth values, and prediction errors for debugging.

## 7. Quick Start: Legacy BJ Workflow
...
| `--sumo-tripinfo <path>` | SUMO `tripinfo.xml` ground-truth output generated with `--tripinfo-output` and aligned by vehicle id for evaluation. | Empty | SUMO |
| `--eval-output <path>` | Optional CSV output for per-vehicle SUMO prediction-vs-truth comparison rows. | Empty | SUMO |
...
## 9. Travel-Time Prediction Modes
...
## 10. Default Paths: Where to Change If Not Using Args
