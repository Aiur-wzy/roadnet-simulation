...
void Graph::nodeID_2_roadID_in_records(vector<vector<pair<int, map<float, vector<vector<int>>>>>> &timeFlowChange) {
    /* timeFlowChange
     vector<vector<pair<int -> node_1, node_2
     map<float, vector<vector<int>>>>>> -> time, vehicle_ID, in/out, flow */

	// Variable Initialization
	route_timeFlowChange.clear();
    map<float, vector<vector<int>>> placeholder;
...
void Graph::split_2_time_slices(vector<map<float, vector<vector<int>>>> &route_timeFlowChange)
{
	...
	for (int i = 0; i < route_timeFlowChange.size(); i++) {
		...
		map<float, vector<vector<int>>>::iterator itr;
		for (itr = route_timeFlowChange[i].begin(); itr != route_timeFlowChange[i].end(); ++itr) {
            float time_float = itr->first;
            int time_int = (int)time_float;
            int hour = time_2_hour(time_int);
            int index = hour_2_index(hour);
            for (int j = 0; j < itr->second.size(); j++) {
                if (timeSlice[RoadSegmentID][index].find(time_int) == timeSlice[RoadSegmentID][index].end()) {
                    // timeSlice[RoadSegmentID][index].insert(pair<int, vector<vector<int>>>(itr->first,{itr->second[j]}));
                    timeSlice[RoadSegmentID][index].insert(pair<int, vector<vector<int>>>(time_int,{itr->second[j]}));

                }
                else {
                    timeSlice[RoadSegmentID][index][time_int].push_back(itr->second[j]);
                }
            }
		}
	}
    ...
    /*
     * On each road segment, the original time records' traffic flow starts from 0 and end to 0.
     * After splitting time records into different time slice, above situation may be changed.
     * This procedure try to add virtual time records to keep this situation under each time slice.
     */
    ...
    for (int i = 0; i < timeSlice.size(); i++) {   // Road
        for (int j = 1; j < timeSlice[i].size(); j++) {    // Time Slice
            ...
            if (timeSlice[i][j-1].size() != 0) {
                ...
                itr = --itr;
                // Last time record under current time slice
            ...
            while (flowRest > 0){
                ...
                if (timeSlice[i][j].find(0) == timeSlice[i][j].end()) {
                    // Virtual time records' ID is set to -1
                    timeSlice[i][j].insert(pair<float, vector<vector<int>>>((0), {{-1, 1, flowInsert}}));
				}
                else {
...
void Graph::ETA_initialization(bool simulation, bool print)
{
	if (simulation == 0) {
		...
		vector<vector<pair<int, float>>> ETA_resultTemp;
		...
		int &RoadSegmentID, int &inTime, pair<int, vector<int>> &newRoutePair, bool &parallel, bool &terminal, bool &range, bool print)
{
    ...
    itFind = timeSliceInsert[RoadSegmentID][index].find(inTime);
	...
	for (int i=0;i<timeRange[current_node].size();i++){
		if(timeRange[current_node][i].first == next_node){
			if (range == false)
                ...
                te = dictionary[{1, 11.18, 11.12, 1, 1, 1, 0, 1, 124}];
            }
		}
	}
						...
						if (trafficFlow != trafficFlowInsert){
							...
							if (sameRange == false or range == false){
								...
								if (range == false){
                                    ...
                                    teCurrent = dictionary[{1, 11.18, 11.12, 1, 1, 1, 0, 1, 124}];
								}
